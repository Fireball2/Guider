...
void CPco_me4::writelog(DWORD lev,PCO_HANDLE hdriver,const char *str,...)
{
 char txt[1000];
 if(log)
 {
...
// Ausgabe auf stdout
//
void CPco_me4::Fg_Error(Fg_Struct *fg)
{
  int error;
  PCO_HANDLE drv;
 // char* str;
...
int CPco_me4::Get_actual_size(unsigned int *width,unsigned int *height,unsigned int *bitpix)
{
  if(width)
   *width=act_width;
  if(height)
...
DWORD CPco_me4::Set_DataFormat(DWORD dataformat)
{
...
DWORD CPco_me4::Set_Grabber_Timeout(int timeout)
{
  int err=PCO_NOERROR;
  int val=timeout;
...
DWORD CPco_me4::Allocate_Framebuffer(int bufnum)
{
...
DWORD CPco_me4::Allocate_Framebuffer(int bufnum,void *adr_in)
{
  int x,err;
  err=PCO_NOERROR;
...
DWORD CPco_me4::Free_Framebuffer()
{
  return Free_Framebuffer(NULL);
}


DWORD CPco_me4::Free_Framebuffer(void *adr_in)
{
  int err;
...
DWORD CPco_me4::Get_Framebuffer()
{
  return nr_of_buffer;
}

DWORD CPco_me4::Start_Aquire(int nr_of_ima)
{
  return Start_Aquire(nr_of_ima,0);
}

DWORD CPco_me4::Start_Aquire(int nr_of_ima,int flag)
{
  int err;
  frameindex_t index;
  ...
  writelog(PROCESS_M,hgrabber,"Start_Aquire: aquire_flag  PCO_SC2_CL_NON_BLOCKING_BUFFER",flag);
...
DWORD CPco_me4::Stop_Aquire()
{
  ...
  writelog(PROCESS_M,hgrabber,"Stop_Acquire: Fg_stopAquire() and Free_Framebuffer done");
  return err;
}

BOOL CPco_me4::started()
{
  return aquire_flag&PCO_SC2_CL_STARTED;
}


DWORD CPco_me4::Wait_For_Image(int *nr_of_pic,int timeout)
{
  int err;
...
DWORD CPco_me4::Get_last_Image(int *nr_of_pic)
{
  int err;
  frameindex_t index=-1;
...
DWORD CPco_me4::Get_Framebuffer_adr(int nr_of_pic,void **adr)
{
  int err;
  err=PCO_NOERROR;
...
DWORD CPco_me4::Check_DMA_Length(int num)
{
  size_t dmaLength;
...
DWORD CPco_me4_edge::Set_Grabber_Size(int width,int height)
{
  int w;
...
DWORD CPco_me4_edge::Open_Grabber(int board)
{
  int err=PCO_NOERROR;
...
//  char *sisodir;
  ...
  strcpy_s(buf, sizeof(buf), "libFullAreaGray8.so");  
...
int CPco_me4_edge::set_sccmos_size(int width,int height,int xlen,int ylen,int xoff,int yoff)
{
  int					rcode, nError;
...
DWORD CPco_me4_edge::Check_DRAM_Status(char *mess,int mess_size,int *fill)
{
...
void CPco_me4_edge::Extract_Image(void *bufout,void *bufin,int width,int height)
{
...
void CPco_me4_edge::Get_Image_Line(void *bufout,void *bufin,int linenumber,int width,int height)
{
  if(pco_hap_loaded==0x03)
  {
   WORD *buf;
...
DWORD CPco_me4_camera::Set_Grabber_Size(int width,int height)
{
  writelog(PROCESS_M,hgrabber,"set_actual_size: start w:%d h:%d",width,height);
...
DWORD CPco_me4_camera::Open_Grabber(int board)
{
  int err=PCO_NOERROR;
...
void CPco_me4_camera::Extract_Image(void *bufout,void *bufin,int width,int height)
{
  memcpy(bufout,bufin,width*height*sizeof(WORD));
}


void CPco_me4_camera::Get_Image_Line(void *bufout,void *bufin,int linenumber,int width,int height)
{
  WORD *buf;
  buf=(WORD*)bufin;
  buf+=(linenumber-1)*width;
